...
\frontmatter
\begin{abstract}
    {全动飞行模拟机是为飞行员提供模拟飞行训练环境的专业设备，能够一比一还原对应型号客机的驾驶体验。
     我国民用航空领域市场广阔，且中国商飞C919客机已然落地，飞行模拟机高度依赖进口的局面亟需打破。
    \par
    本项目着眼于提供飞行图像的视景系统中数据交换子系统的设计实现。
    仿真机是一台模拟机的核心，视景系统的运作需要仿真机的指令驱动。
    然而进口仿真机并不对外开放接口，为了基于其开发视景系统，本文首先通过网络流量分析的方法结合文档验证了仿真机网络最高层为数据链路层，
    ...
    目前该视景系统可以在CAE仿真机操控下，以至少60帧率实现地景数据库中各机场附近环绕飞行，标志着迈出了动起来的坚实第一步。以此为根基，后续对天气、灯光、植被等多种效果进行研究与引入，尽早达成训练用模拟机的验收标准。
    }          
